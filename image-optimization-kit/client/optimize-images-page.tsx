/**
 * ══════════════════════════════════════════════════════════════
 *  OPTIMIZATION DASHBOARD — Client Page Component
 * ══════════════════════════════════════════════════════════════
 * 
 *  Full admin page for analyzing and optimizing existing images.
 *  
 *  Features:
 *    - Lists all images from the product_images table
 *    - Analyzes file sizes (downloads each to measure)
 *    - Optimizes images one-by-one or in bulk (compress via sharp)
 *    - SELECT specific images to analyze/optimize (checkboxes)
 *    - Dry-run test mode for debugging
 *    - Color-coded size indicators (red > 200 Ko, green ≤ 200 Ko)
 * 
 *  This file should be placed at: src/app/admin/optimize-images/page.tsx
 * 
 *  It imports server actions from: src/app/actions/optimize.ts
 *    - getImagesToOptimize()
 *    - optimizeSingleImage(id, url)
 *    - getImageSize(url)
 *    - dryRunOptimize(url)
 * 
 *  ADAPT: Update the import path if your actions file is elsewhere.
 * ══════════════════════════════════════════════════════════════
 */
...
// ADAPT: Change this import path to match where you placed optimize.ts
...
export default function OptimizeImagesPage() {
  ...
  async function handleAnalyzeAll() {
    ...
    setLatestLog(`Analyse terminee ! ${images.length} images analysees.`);
  ...
  async function handleAnalyzeSelected() {
    ...
    setLatestLog(`Analyse terminee ! ${count} image(s) analysee(s).`);
    setIsAnalyzingAll(false);
  }
  ...
  async function handleOptimize(index: number) {
    ...
    setLatestLog(`Optimisation de "${item.products?.name || 'Image'}" (${item.color || ''})... Tout se passe sur le serveur.`);
    ...
    try {
      ...
      if (result.success) {
        ...
        setLatestLog(`Succes ! "${item.products?.name || 'Image'}" : ${result.savedText}\nURL sauvegardee: ${result.newUrl}`);
      } else {
        setImages(prev => {
          const copy = [...prev];
        ...
        setLatestLog(`Echec "${item.products?.name || 'Image'}" : ${result.error}`);
      }
    } catch (err: any) {
      setImages(prev => {
        const copy = [...prev];
        copy[index] = { ...copy[index], status: 'error', error: err.message || 'Erreur reseau' };
        return copy;
      });
      setLatestLog(`Echec "${item.products?.name || 'Image'}" : ${err.message || 'Erreur reseau'}`);
    }
  }
  ...
  async function handleOptimizeAll() {
    ...
    setLatestLog(`Termine ! ${images.filter(i => i.status === 'success').length} images optimisees.`);
  ...
  async function handleOptimizeSelected() {
    ...
    setLatestLog(`Termine ! ${done} image(s) sélectionnée(s) optimisée(s).`);
    setIsProcessingAll(false);
  }
  ...
  async function handleDryRun(index: number) {
    ...
    try {
      ...
      if (result.success) {
        ...
        setLatestLog(`TEST ECHOUE: ${result.error}\n${result.debug || ''}`);
      }
    } catch (err: any) {
  ...
  return (
    <div style={{ padding: '24px', maxWidth: '1000px', margin: '0 auto', fontFamily: 'system-ui, sans-serif' }}>
      ...
      <p style={{ color: '#6b7280', fontSize: '0.9rem', marginBottom: '20px' }}>
        Tout le traitement se fait sur le serveur avec <strong>sharp</strong>. Rien ne passe par votre navigateur.
      </p>

      {/* Toolbar */}
      <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap', marginBottom: '16px', alignItems: 'center' }}>
        {/* Selection-specific actions (only visible when images are selected) */}
        {hasSelection && (
          <>
            <button
        ...
        <span style={{ alignSelf: 'center', fontSize: '0.85rem', color: '#374151', fontWeight: 600 }}>
          {images.length} images | {successCount} optimisees
      ...
      {latestLog && (
        <div style={{
          padding: '12px 16px',
          backgroundColor: latestLog.startsWith('Echec') ? '#fef2f2' : '#eff6ff',
          color: latestLog.startsWith('Echec') ? '#b91c1c' : '#1e40af',
          border: `1px solid ${latestLog.startsWith('Echec') ? '#fca5a5' : '#bfdbfe'}`,
            ...
            {images.map((img, index) => {
              ...
              return (
                ...
                >
                  ...
                  <td style={{ padding: '8px 10px', fontWeight: 500, maxWidth: '120px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                    {img.products?.name || '?'}
                  </td>
                  <td style={{ padding: '8px 10px' }}>
